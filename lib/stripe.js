...
// Get Stripe metrics for connected account
export async function getStripeMetrics(userId, days = 30) {
  const res = await fetch(`/api/connect/stripe/metrics?userId=${encodeURIComponent(userId)}&days=${days}`);
  if (!res.ok) {
    if (res.status === 404) {
      // Return null if no connected account or metrics
      return null;
    }
