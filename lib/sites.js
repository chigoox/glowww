...
const validateSubdomain = (subdomain) => {
  const cleaned = subdomain.toString().trim().toLowerCase();
  ...
  if (!/^[a-z0-9][a-z0-9-]*[a-z0-9]$|^[a-z0-9]$/.test(cleaned)) {
    throw new Error('Subdomain can only contain letters, numbers, and hyphens (cannot start or end with hyphen)');
  }
...
export const canCreateSite = async (userId) => {
  try {
    console.log('canCreateSite called for userId:', userId);
    // Get subscription & current stored usage
    const subscription = await getUserSubscription(userId);
    const { limits, usage } = subscription;
    console.log('User subscription:', { tier: subscription.tier, limits, usage });
    ...
    try {
      ...
      console.log('Real site count:', realCount);
    } catch (e) {
      console.log('Error fetching existing sites:', e);
    ...
    console.log('Effective site count:', effectiveCount, 'maxSites:', limits.maxSites);
    ...
    console.log('Checking limits - tier:', subscription.tier, 'maxSites:', limits.maxSites, 'effectiveCount:', effectiveCount);
    if (subscription.tier !== 'admin' && limits.maxSites !== -1 && effectiveCount >= limits.maxSites) {
      console.log('Site limit exceeded for non-admin user');
      return {
        allowed: false,
    ...
    console.log('Limit check passed, checking additional permissions...');
    ...
    console.log('checkUserLimits result:', canCreate);
    ...
    console.log('All checks passed, allowing site creation');
    return {
      allowed: true,
      currentCount: effectiveCount,
      maxSites: limits.maxSites === -1 ? Infinity : limits.maxSites
...
export const createSite = async (userId, siteData) => {
  try {
    console.log('createSite called with userId:', userId, 'siteData:', siteData);
    // Subscription & real-count based guard (defense-in-depth)
    const subscription = await getUserSubscription(userId);
    const { limits } = subscription;
    console.log('User subscription in createSite:', { tier: subscription.tier, limits });

    // Get existing sites early for limit validation & main-site detection
    const userSites = await getUserSites(userId);
    console.log('Existing sites count:', userSites.length);

    // Skip limit check for admin users
    if (subscription.tier !== 'admin' && limits.maxSites !== -1 && userSites.length >= limits.maxSites) {
      console.log('Site limit reached for non-admin user');
      throw new Error(`Site limit reached (${limits.maxSites}). Upgrade for more sites.`);
    }
    console.log('Site limit check passed');

    // Also invoke standardized limit checker (may include feature gating)
    const canCreate = await checkUserLimits(userId, 'create_site');
    console.log('checkUserLimits result in createSite:', canCreate);
    if (!canCreate.allowed) {
