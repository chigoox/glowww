...
export const canCreateSite = async (userId) => {
  try {
    // Use the new subscription system to check site creation limits
    const canCreate = await checkUserLimits(userId, 'sites');
    
    if (!canCreate.allowed) {
      return {
        allowed: false,
        reason: canCreate.reason,
        currentCount: canCreate.currentUsage,
        maxSites: canCreate.limit === -1 ? Infinity : canCreate.limit
      };
    }
...
export const createSite = async (userId, siteData) => {
  try {
    // Check if user can create more sites using the new subscription system
    const canCreate = await checkUserLimits(userId, 'sites');
    ...
    await updateUserUsage(userId, 'sites');
...
export const deleteSite = async (userId, siteId) => {
  try {
    ...
    await updateUserUsage(userId, 'sites', { operation: 'decrement' });
