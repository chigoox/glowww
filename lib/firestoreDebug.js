...
// Include a broad set used across the codebase so consumers don't see missing-symbol errors.
export const {
  doc,
  collection,
  query,
...
export function onSnapshotDebug(refOrQuery, ...rest) {
  ...
  const okKinds = ['Query', 'QueryLike', 'CollectionReferenceLike', 'DocumentReference', 'DocumentReferenceLike'];
  // Avoid repeatedly logging the same object - use WeakSet for object identity
  if (!globalThis.__firestore_onSnapshot_warned) globalThis.__firestore_onSnapshot_warned = new WeakSet();
  const warnedSnapshot = globalThis.__firestore_onSnapshot_warned;
  if (!okKinds.includes(kind)) {
    // Only log once per object to reduce noise
    if (refOrQuery && typeof refOrQuery === 'object') {
      if (!warnedSnapshot.has(refOrQuery)) {
        warnedSnapshot.add(refOrQuery);
        // eslint-disable-next-line no-console
        console.warn('[firestoreDebug] onSnapshot arg kind=', kind, {
          kind,
          keys: Object.keys(refOrQuery).slice(0,10),
          stack: new Error().stack?.split('\n').slice(0,8).join('\n')
        });
      }
    } else {
      // For non-objects, log once globally
      if (!globalThis.__firestore_onSnapshot_primitive_warned) {
        globalThis.__firestore_onSnapshot_primitive_warned = true;
        // eslint-disable-next-line no-console
        console.warn('[firestoreDebug] onSnapshot arg kind=', kind, { kind, stack: new Error().stack?.split('\n').slice(0,8).join('\n') });
      }
    }
  }
