...
placeContent,
  ...
  // Safe numeric min constraints (respect explicit 0 instead of falling back)
  const safeMinWidth = (() => {
...
placeContent,
  ...
  return (
    <div
  className={`gl-canvas nodeID-${nodeId} ${isSelected && !hideEditorUI ? 'ring-2 ring-blue-500' : ''} ${isHovered && !hideEditorUI ? 'ring-1 ring-gray-300' : ''} ${isMultiSelected(nodeId) ? 'ring-2 ring-purple-500 multi-selected-element' : ''} ${className || ''}`}
  data-gl-canvas="true"
  data-component="Box"
      ref={cardRef}
      style={{
