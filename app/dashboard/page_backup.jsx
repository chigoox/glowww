...
export default function Dashboard() {
  ...
  // Initialize active tab from URL (?tab=overview|sites|payments|analytics)
  useEffect(() => {
  ...
  const handleViewAnalytics = (site) => {
    // Navigate to analytics tab with site pre-selected
    setActiveTab('analytics');
    // If using a more sophisticated analytics component, we could pass the site ID
    // For now, the analytics dashboard will let users select the site
  };

  const handleManageEmails = (site) => {
    // Navigate to emails tab with site pre-selected
    setActiveTab('emails');
    // You could store the selected site in state if EmailManager needs it
    // For now, users can filter by site in the EmailManager interface
  };
            ...
            items={[
              {
                key: 'site-analytics',
                label: (
                  <span><BarChartOutlined /> Site Analytics</span>
                ),
                children: <UserAnalyticsDashboard />
              },
              {
                key: 'payment-analytics',
                label: (
                  <span><LineChartOutlined /> Payment Analytics</span>
                ),
                children: (
                    ...
                    >
                      {stripeMetricsLoading ? (
                        ...
                        <Alert
                          ...
                          description="Connect with Stripe and complete some transactions to see insights."
                          type="info"
                          showIcon
                        />
                      )}
